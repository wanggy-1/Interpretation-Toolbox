...
seismic_file = '/nfs/shengli2020/Niuzhuang/Post-stack seismic/nz.sgy'
# If multiple files.
...
for n in range(Nfile):
    with segyio.open(seismic_file) as f:
        print('Read seismic data from file: ', seismic_file)
        ...
        for i in range(f.tracecount):
            ...
            x[i] = f.header[i][73] * 1e-2
            y[i] = f.header[i][77] * 1e-2
        sys.stdout.write('\n')
...
df_ctp = pd.DataFrame()
abnormal_value = -999  # Abnormal log value.
if vertical_well:
    # Read well locations.
...
for log_file in log_list:
    ...
    df = resample_log(df, delta=dt, depth_col=depth_name, log_col=log_name, method='most_frequent')
    if vertical_well:
        # Extract well coordinates from well (only for vertical wells).
        well_coord = df_loc.loc[df_loc['well_name'] == log_file[:-4], ['well_X', 'well_Y']].values
...
if mode == 'memory-saving':
    ...
    sys.stdout.write('\n')
# if mode == 'time-saving':
#     # Compute distance map in time-slice direction.
#     for i in range(cube_itp.shape[2]):  # Time-slice.
#         sys.stdout.write('\rInterpolating trace %d/%d [%.2f%%]' %
#                          (i + 1, cube_itp.shape[2], (i+1) / cube_itp.shape[2] * 100))
#         x_itp = x.ravel(order='C')  # x-coordinate.
#         y_itp = y.ravel(order='C')  # y-coordinate.
#         t_itp = t_cut[i] * np.ones(len(x_itp))  # Two-way time.
#         seis_itp = np.zeros(shape=[len(x_itp), Nfile], dtype='float32')  # Seismic attributes.
#         for n in range(Nfile):
#             seis_itp[:, n] = seis[n][:, :, i+ind_min].ravel(order='C')
#         itp = np.c_[x_itp, y_itp, t_itp, seis_itp]  # Points to interpolate.
#         if scale:
#             itp[:, 0:2] = xy_scalar.transform(itp[:, 0:2])  # Scale interpolate points' x and y coordinates.
#             itp[:, 2] = np.squeeze(t_scalar.transform(itp[:, 2].reshape(-1, 1)))  # Scale interpolate points' TWT.
...
#         if log_itp.ndim == 1:
#             log_itp = log_itp.reshape(-1, 1)
#         cube_itp[:, :, i, :] = log_itp.reshape([cube_itp.shape[0], cube_itp.shape[1], log_itp.shape[1]], order='C')
...
for i in range(cube_itp.shape[3]):
    file_out = log_name[i] + '.txt'
    sys.stdout.write('Writing interpolation result to file %s...' % file_out)
    cube_out = cube_itp[:, :, :, i].reshape([cube_itp.shape[0] * cube_itp.shape[1], cube_itp.shape[2]], order='C')
    np.savetxt(file_out, np.c_[x_out, y_out, cube_out], delimiter='\t')
